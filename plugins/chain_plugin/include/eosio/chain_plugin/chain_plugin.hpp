...
public:
   ...
   struct get_xshards_params {
      string                  lower_bound;
      uint32_t                limit = 50;
      std::optional<uint32_t> time_limit_ms; // defaults to 10ms
   };

   struct get_xshards_results {
      vector<fc::variant> xshards;
      string              more; ///< fill lower_bound with this value to fetch more rows
   };

   get_xshards_results get_xshards( const get_xshards_params& params, const fc::time_point& deadline )const;
...
FC_REFLECT( eosio::chain_apis::read_only::get_xshards_params, (lower_bound)(limit)(time_limit_ms) )
FC_REFLECT( eosio::chain_apis::read_only::get_xshards_results, (xshards)(more) )
