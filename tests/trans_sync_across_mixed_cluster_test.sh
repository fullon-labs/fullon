...
getHeadBlockNum()
{
  INFO="$(bin/focli get info)"
  verifyErrorCode "focli get info"
...
# stand up fonod cluster
launcherOpts="-p $pnodes -n $total_nodes -s $topo -d $delay"
echo Launcher options: --fonod \"--plugin eosio::wallet_api_plugin\" $launcherOpts
programs/flon-launcher/flon-launcher --fonod "--plugin eosio::wallet_api_plugin" $launcherOpts
sleep 7
...
while [ $port2  -ne $endport ]; do
    ...
    TRANS_INFO="$(bin/focli --port $port2 get block 1)"
    verifyErrorCode "focli get block"
    port2=`expr $port2 + 1`
done

# create 3 keys
KEYS="$(bin/focli create key)"
verifyErrorCode "focli create key"
...
PASSWORD_INITA="$(bin/focli wallet create --name inita)"
verifyErrorCode "focli wallet create"
# strip out password from output
PASSWORD_INITA="$(echo "$PASSWORD_INITA" | awk '/PW/ {print $1}')"
# remove leading/trailing quotes
PASSWORD_INITA=${PASSWORD_INITA#\"}
PASSWORD_INITA=${PASSWORD_INITA%\"}
bin/focli wallet import --name inita --private-key $INITA_PRV_KEY
verifyErrorCode "focli wallet import"
bin/focli wallet import --name inita --private-key $PRV_KEY1
verifyErrorCode "focli wallet import"
bin/focli wallet import --name inita --private-key $PRV_KEY2
verifyErrorCode "focli wallet import"
bin/focli wallet import --name inita --private-key $PRV_KEY3
verifyErrorCode "focli wallet import"
...
ACCOUNT_INFO="$(bin/focli create account inita testera $PUB_KEY1 $PUB_KEY3)"
verifyErrorCode "focli create account"
waitForNextBlock
# verify account created
ACCOUNT_INFO="$(bin/focli get account testera)"
verifyErrorCode "focli get account"
...
while [ $port  -ne $endport ]; do
    ...
    TRANSFER_INFO="$(bin/focli transfer inita testera 975321 "test transfer")"
    verifyErrorCode "focli transfer"
   ...
   TRANS_INFO="$(bin/focli --port $port2 get transaction $TRANS_ID)"
   verifyErrorCode "focli get transaction trans_id of <$TRANS_INFO> from node on port $port2"
	port2=`expr $port2 + 1`
    done
