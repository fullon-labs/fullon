...
to encapsulate the configurations options available for each plugin as currently documented in fonod's --help command.

It makes use of the compiled fonod program and runs the --help command, capturing the output.
...
Each *PluginArgs.py file contains one dataclass that captures the available configuration options for that plugin via fonod command line.

Each config option is represented by 3 member variables, for example:
1) blocksDir: str=None
    --This is the field that will be populated when the dataclass is used by other scripts to configure fonod
2) _blocksDirNodeosDefault: str='"blocks"'
    --This field captures the default value in the fonod output.  This will be compared against the first field to see if the configuration
    option will be required on the command line to override the default value when running fonod.
...
2) __str__
    -- Provides the command line argument string for the current configuration to pass to fonod
...
- There are some hardcoded edge cases when trying to determine the types associated with certain default argument parameters.
  These may need to be updated to account for new/different options as they are added/removed/modified by fonod

Note:
- To help with maintainability the validate_fonod_plugin_args.py test script is provided which validates the current
  *PluginArgs dataclass configuration objects against the current fonod --help output to notify developers when
...
def main():
    ...
    def writeDataclass(plugin:str, dataFieldDict:dict, pluginOptsDict:dict):
        ...
        with open(pluginArgsFile, 'w') as dataclassFile:
            ...
            dataclassFile.write(f"This file/class was generated by generate_fonod_plugin_args_class_files.py\n")
            dataclassFile.write(f"\"\"\"\n\n")
