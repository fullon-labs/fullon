...
#
# This test sets up  a cluster with 21 producers fonod, each fonod is configured with only one producer and only connects to the bios node.
# Moreover, each producer fonod is also configured with a list of p2p-auto-bp-peer so that each one can automatically establish p2p connections to
