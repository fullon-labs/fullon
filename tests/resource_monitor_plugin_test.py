...
# We need debug level to get more information about fonod process
logging="""{
  "includes": [],
  "appenders": [{
      "name": "stderr",
...
def runNodeos(extraNodeosArgs, myTimeout):
    """Startup fonod, wait for timeout (before forced shutdown) and collect output."""
    if debug: Print("Launching fonod process.")
    cmd="bin/fonod --config-dir rsmStaging/etc -e -p flon --plugin eosio::chain_api_plugin --data-dir " + dataDir + " "
...
def testCommon(title, extraNodeosArgs, expectedMsgs):
    ...
    timeout=120  # Leave sufficient time such fonod can start up fully in any platforms
...
testIntervalMaxTimeout = 300 # Assume fonod at most runs 300 sec for this test
...
def testInterval(title, extraNodeosArgs, interval, expectedMsgs, warningThreshold):
    ...
    timeout = 120 + interval * 2 # Leave sufficient time so fonod can start up fully in any platforms, and at least two warnings can be output
