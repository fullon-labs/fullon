...
class PluginHttpTest(unittest.TestCase):
    ...
    node_id = 1
    gaxnod = Node(TestHelper.LOCAL_HOST, TestHelper.DEFAULT_PORT, node_id, walletMgr=gaxkey)
    ...
    # kill gaxnod and gaxkey and clean up dirs
    def cleanEnv(self) :
    ...
    def activateAllBuiltinProtocolFeatures(self):
        self.gaxnod.activatePreactivateFeature()
        ...
        retMap = self.gaxnod.publishContract(eosioAccount, contractDir, wasmFile, abiFile, waitForTransBlock=True)

        self.gaxnod.preactivateAllBuiltinProtocolFeature()
    ...
    def test_ChainApi(self) :
        ...
        ret_json = self.gaxnod.processUrllibRequest(resource, command)
        self.assertIn("server_version", ret_json["payload"])
        # get_info with empty content parameter
        ret_json = self.gaxnod.processUrllibRequest(resource, command, self.empty_content_dict)
        self.assertIn("server_version", ret_json["payload"])
        # get_info with invalid parameter
        ret_json = self.gaxnod.processUrllibRequest(resource, command, self.http_post_invalid_param)
        self.assertEqual(ret_json["code"], 400)

        # activate the builtin protocol features and get some useful data
        self.activateAllBuiltinProtocolFeatures()
        allProtocolFeatures = self.gaxnod.getSupportedProtocolFeatures()
        ...
        payload_no_actions = {"expiration":"2020-08-01T07:15:49",
                ...
                "context_free_data": []}
        ret_json = self.gaxnod.processUrllibRequest(resource, command, payload_no_actions)
        self.assertEqual(ret_json["code"], 400)
        self.assertEqual(ret_json["error"]["code"], 3200006)
        ...
        payload_invalid_actions = {"expiration":"2020-08-01T07:15:49",
                ...
                "signatures": ["SIG_K1_KeqfqiZu1GwUxQb7jzK9Fdks6HFaVBQ9AJtCZZj56eG9qGgvVMVtx8EerBdnzrhFoX437sgwtojf2gfz6S516Ty7c22oEp"],
                "context_free_data": []}
        ret_json = self.gaxnod.processUrllibRequest(resource, command, payload_invalid_actions)
        self.assertEqual(ret_json["code"], 400)
        self.assertEqual(ret_json["error"]["code"], 3200006)
        ...
        payload = {"expiration":"2020-08-01T07:15:49",
                   ...
                   "signatures": ["SIG_K1_KeqfqiZu1GwUxQb7jzK9Fdks6HFaVBQ9AJtCZZj56eG9qGgvVMVtx8EerBdnzrhFoX437sgwtojf2gfz6S516Ty7c22oEp"],
                   "context_free_data": []}
        ret_str = self.gaxnod.processUrllibRequest(resource, command, payload, returnType=ReturnType.raw).decode('ascii')
        ...
        payload_hex = {"expiration":"2020-08-01T07:15:49",
                ...
                "signatures": ["SIG_K1_KeqfqiZu1GwUxQb7jzK9Fdks6HFaVBQ9AJtCZZj56eG9qGgvVMVtx8EerBdnzrhFoX437sgwtojf2gfz6S516Ty7c22oEp"],
                "context_free_data": []}
        ret_str = self.gaxnod.processUrllibRequest(resource, command, payload_hex, returnType=ReturnType.raw).decode('ascii')
        self.assertEqual(ret_str, "\"0be762a6406bab15530e87f21e02d1c58e77944ee55779a76f4112e3b65cac48\"")
    ...
    def test_NetApi(self) :
        ...
        command = "connections"
        ret_str = self.gaxnod.processUrllibRequest(resource, command, returnType=ReturnType.raw).decode('ascii')
        self.assertIn("\"peer\":\"localhost:9011\"", ret_str)
        # connections with empty content parameter
        ret_str = self.gaxnod.processUrllibRequest(resource, command, self.empty_content_dict, returnType=ReturnType.raw).decode('ascii')
        self.assertIn("\"peer\":\"localhost:9011\"", ret_str)
    ...
    def test_WalletApi(self) :
        ...
        command = "set_timeout"
        ret_json = self.gaxnod.processUrllibRequest(resource, command, endpoint=endpoint)
        self.assertEqual(ret_json["code"], 400)
        self.assertEqual(ret_json["error"]["code"], 3200006)
        # set_timeout with empty content parameter
        ret_json = self.gaxnod.processUrllibRequest(resource, command, self.empty_content_dict, endpoint=endpoint)
        self.assertEqual(ret_json["code"], 400)
        self.assertEqual(ret_json["error"]["code"], 3200006)
        # set_timeout with invalid parameter
        ret_json = self.gaxnod.processUrllibRequest(resource, command, self.http_post_invalid_param, endpoint=endpoint)
        self.assertEqual(ret_json["code"], 400)
        self.assertEqual(ret_json["error"]["code"], 3200006)
        # set_timeout with valid parameter
        payload = "1234567"
        ret_str = self.gaxnod.processUrllibRequest(resource, command, payload, returnType=ReturnType.raw, endpoint=endpoint).decode('ascii')
        ...
        ret_json = self.gaxnod.processUrllibRequest(resource, command, payload, endpoint=endpoint)
        self.assertEqual(ret_json["code"], 500)
        ...
        command = "lock_all"
        ret_str = self.gaxnod.processUrllibRequest(resource, command, returnType=ReturnType.raw, endpoint=endpoint).decode('ascii')
        self.assertEqual("{}", ret_str)
        # lock_all with empty content parameter
        ret_str = self.gaxnod.processUrllibRequest(resource, command, self.empty_content_dict, returnType=ReturnType.raw, endpoint=endpoint).decode('ascii')
        self.assertEqual("{}", ret_str)
    ...
    def test_prometheusApi(self) :
        resource = "prometheus"
        command = "metrics"
        endpointPrometheus = f'http://{self.gaxnod.host}:9101'
        ret_text = self.gaxnod.processUrllibRequest(resource, command, returnType = ReturnType.raw, method="GET", endpoint=endpointPrometheus).decode()
        ...
        ret = self.gaxnod.processUrllibRequest(resource, "m", returnType = ReturnType.raw, method="GET", silentErrors= True, endpoint=endpointPrometheus)
    ...
    def test_multipleRequests(self):
        ...
        host = self.gaxnod.host
        port = self.gaxnod.port
        addr = (host, port)
        ...
        try:
            ...
            print(e)
            Utils.errorExit("Failed to connect to gaxnod.")
