...
class resource_limits_fixture: private database_manager_fixture<1024*1024>, public resource_limits_manager
{
   public:
      ...
      chainbase::database& get_shard() { return (*database_manager_fixture::_dbm).main_db();}
      chainbase::database& get_shared() { return (*database_manager_fixture::_dbm).main_db();}
};
...
BOOST_AUTO_TEST_SUITE(resource_limits_test)
   ...
   BOOST_FIXTURE_TEST_CASE(elastic_cpu_relax_contract, resource_limits_fixture) try {
      ...
      set_account_limits(account, -1, -1, -1, shared_db, false);
      ...
      while( get_virtual_block_cpu_limit() < desired_virtual_limit && iterations <= expected_relax_iterations ) {
         add_transaction_usage({account},0,0,iterations,shard_db,shared_db);
      ...
      while( get_virtual_block_cpu_limit() > config::default_max_block_cpu_usage
         ...
         add_transaction_usage({account}, config::default_max_block_cpu_usage, 0, iterations, shard_db, shared_db );
         process_block_usage(iterations++);
      }
   ...
   BOOST_FIXTURE_TEST_CASE(elastic_net_relax_contract, resource_limits_fixture) try {
      ...
      while( get_virtual_block_net_limit() > config::default_max_block_net_usage
              && iterations <= expected_relax_iterations + expected_contract_iterations ) {
         add_transaction_usage({account},0, config::default_max_block_net_usage, iterations, shard_db, shared_db);
         process_block_usage(iterations++);
      }
...
// TODO: restore weighted capacity cpu tests
   ...
   BOOST_FIXTURE_TEST_CASE(weighted_capacity_net, resource_limits_fixture) try {
      ...
      for (int64_t idx = 0; idx < weights.size(); idx++) {
         const account_name account(idx + 100);
         initialize_account(account, false);
         set_account_limits(account, -1, weights.at(idx), -1, shared_db, false );
      }
      ...
      for (int64_t idx = 0; idx < weights.size(); idx++) {
         ...
         BOOST_REQUIRE_THROW(add_transaction_usage({account}, 0, expected_limits.at(idx) + 1, 0, db, shared_db), tx_net_usage_exceeded);
      }
   } FC_LOG_AND_RETHROW();
#endif
   ...
   BOOST_FIXTURE_TEST_CASE(enforce_block_limits_cpu, resource_limits_fixture) try {
      ...
      for (uint64_t idx = 0; idx < expected_iterations; idx++) {
         add_transaction_usage({account}, increment, 0, 0, shard_db, shared_db);
      }

      BOOST_REQUIRE_THROW(add_transaction_usage({account}, increment, 0, 0, shard_db, shared_db), block_resource_exhausted);
   ...
   BOOST_FIXTURE_TEST_CASE(enforce_block_limits_net, resource_limits_fixture) try {
      ...
      for (uint64_t idx = 0; idx < expected_iterations; idx++) {
         add_transaction_usage({account}, 0, increment, 0, shard_db, shared_db);
      }

      BOOST_REQUIRE_THROW(add_transaction_usage({account}, 0, increment, 0, shard_db, shared_db), block_resource_exhausted);
   ...
   BOOST_FIXTURE_TEST_CASE(enforce_account_ram_limit, resource_limits_fixture) try {
      ...
      const account_name account(1);
      initialize_account(account, false);
      set_account_limits(account, limit, -1, -1, shared_db,false);
      process_account_limit_updates();

      for (uint64_t idx = 0; idx < expected_iterations - 1; idx++) {
         add_pending_ram_usage(account, increment, shard_db);
         verify_account_ram_usage(account, shard_db, shared_db);
      }

      add_pending_ram_usage(account, increment, shard_db);
      BOOST_REQUIRE_THROW(verify_account_ram_usage(account, shard_db, shared_db), ram_usage_exceeded);
   } FC_LOG_AND_RETHROW();
   ...
   BOOST_FIXTURE_TEST_CASE(enforce_account_ram_limit_underflow, resource_limits_fixture) try {
      ...
      chainbase::database& shard_db = get_shard();
      chainbase::database& shared_db = get_shared();
      set_account_limits(account, 100, -1, -1, shared_db, false);
      verify_account_ram_usage(account, shard_db, shared_db );
      process_account_limit_updates();
      BOOST_REQUIRE_THROW(add_pending_ram_usage(account, -101, shard_db), transaction_exception);
   ...
   BOOST_FIXTURE_TEST_CASE(enforce_account_ram_limit_overflow, resource_limits_fixture) try {
      ...
      chainbase::database& shard_db = get_shard();
      chainbase::database& shared_db = get_shared();
      set_account_limits(account, UINT64_MAX, -1, -1, shared_db,false);
      verify_account_ram_usage(account, shard_db, shared_db);
      process_account_limit_updates();
      add_pending_ram_usage(account, UINT64_MAX/2, shard_db);
      verify_account_ram_usage(account, shard_db, shared_db);
      add_pending_ram_usage(account, UINT64_MAX/2, shard_db);
      verify_account_ram_usage(account, shard_db, shared_db);
      BOOST_REQUIRE_THROW(add_pending_ram_usage(account, 2, shard_db), transaction_exception);
   ...
   BOOST_FIXTURE_TEST_CASE(enforce_account_ram_commitment, resource_limits_fixture) try {
      ...
      set_account_limits(account, limit, -1, -1, shared_db,false);
      process_account_limit_updates();
      add_pending_ram_usage(account, commit, shard_db);
      verify_account_ram_usage(account, shard_db, shared_db);

      for (int idx = 0; idx < expected_iterations - 1; idx++) {
         set_account_limits(account, limit - increment * idx, -1, -1, shared_db, false);
         verify_account_ram_usage(account, shard_db, shared_db);
         process_account_limit_updates();
      }

      set_account_limits(account, limit - increment * expected_iterations, -1, -1, shared_db, false);
      BOOST_REQUIRE_THROW(verify_account_ram_usage(account, shard_db, shared_db), ram_usage_exceeded);
   } FC_LOG_AND_RETHROW();
   ...
   BOOST_FIXTURE_TEST_CASE(sanity_check, resource_limits_fixture) try {
      ...
      set_account_limits( "dan"_n, 0, 0, user_stake, shared_db, false );
      set_account_limits( "everyone"_n, 0, 0, (total_staked_tokens - user_stake), shared_db, false );
      process_account_limit_updates();

      // dan cannot consume more than 34 us per day
      BOOST_REQUIRE_THROW( add_transaction_usage( {"dan"_n}, 35, 0, 1, shard_db, shared_db ), tx_cpu_usage_exceeded );

      // Ensure CPU usage is 0 by "waiting" for one day's worth of blocks to pass.
      add_transaction_usage( {"dan"_n}, 0, 0, 1 + blocks_per_day, shard_db, shared_db );

      // But dan should be able to consume up to 34 us per day.
      add_transaction_usage( {"dan"_n}, 34, 0, 2 + blocks_per_day, shard_db, shared_db );
   } FC_LOG_AND_RETHROW()
   ...
   BOOST_FIXTURE_TEST_CASE(get_account_net_limit_ex_and_get_account_cpu_limit_ex, resource_limits_fixture) try {
      ...
      using get_account_limit_ex_func = std::function<std::pair<account_resource_limit, bool>(const resource_limits_manager*, const account_name&, const chainbase::database& shard_db, const chainbase::database& shared_db, uint32_t, const std::optional<block_timestamp_type>&)>;
      auto test_get_account_limit_ex = [this](const account_name& test_account, const uint32_t window, get_account_limit_ex_func get_account_limit_ex)
      {
         ...
         set_account_limits(test_account, unlimited, unlimited, unlimited, shared_db, false);
         process_account_limit_updates();
         // unlimited
         {
         ...
         set_account_limits(test_account, -1, net_limit, cpu_limit, shared_db, false);
         process_account_limit_updates();
         ...
         add_transaction_usage({test_account}, cpu_usage, net_usage, update_slot, shard_db, shared_db );
