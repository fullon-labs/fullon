...
BOOST_AUTO_TEST_CASE( billed_cpu_test ) try {
   ...
   auto push_trx = [&]( const transaction_metadata_ptr& trx, fc::time_point deadline,
      ...
      auto r = chain.push_transaction( trx, deadline, fc::microseconds::maximum(), billed_cpu_time_us, explicit_billed_cpu_time, subjective_cpu_bill_us );
   ...
   cpu_limit = mgr.get_account_cpu_limit_ex_readonly(acc, db, shared_db).first.max;
   ...
   chain.produce_block( fc::days(1) ); // produce for one day to reset account cpu
   ptrx = create_trx(0);
   uint32_t combined_cpu_limit = mgr.get_account_cpu_limit_ex_readonly(acc, db, shared_db).first.max + leeway.count();
   ...
   chain.produce_block( fc::days(1) ); // produce for one day to reset account cpu
   ptrx = create_trx(0);
   combined_cpu_limit = mgr.get_account_cpu_limit_ex_readonly(acc, db, shared_db).first.max + leeway.count();
