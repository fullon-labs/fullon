...
namespace eosio {
   ...
   class [[eosio::contract("eosio.token")]] token : public contract {
      ...
      public:
         ...
         /**
          * Allows `owner` account to make a request to transfer from current shard to `to_shard` the `quantity` tokens.
          * The `owner` account of current shard is debited with `quantity` tokens,
          * and `owner` must use `xshin` action to credit the tokens on `to_shard`.
          *
         ...
         /**
          * Allows `owner` account of current shard to accept the `quantity` tokens transfered from `from_shard`.
          * The `owner` account is credited with `quantity` tokens,
          *
          * @param owner - the owner account to execute this action,
          * @param from_shard - the shard to be transferred to,
          * @param quantity - the quantity of tokens to be transferred,
          * @param memo - the memo string to accompany the transaction.
          *
         ...
         using xshout_action = eosio::action_wrapper<"xshout"_n, &token::xshout>;
         using xshin_action = eosio::action_wrapper<"xshin"_n, &token::xshin>;
      private:
