...
BOOST_AUTO_TEST_CASE( replace_account_keys ) try {
   ...
   auto& main_db = const_cast<chainbase::database&>(tester.control->dbm().main_db());
   const auto old_ram_usg = rlm.get_account_ram_usage(usr, main_db);
   ...
   const auto new_ram_usg = rlm.get_account_ram_usage(usr, main_db);
