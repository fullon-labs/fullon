...
BOOST_AUTO_TEST_CASE( replace_account_keys ) try {
   ...
   const auto old_ram_usg = rlm.get_account_ram_usage(usr, tester.control->dbm().main_db());
   ...
   const auto new_ram_usg = rlm.get_account_ram_usage(usr, tester.control->dbm().main_db());
