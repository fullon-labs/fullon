...
BOOST_AUTO_TEST_CASE(light_validation_skip_cfa) try {
   ...
   cfg.trusted_producers = { "flon"_n }; // light validation
...
void push_trx(Tester& test, T ac, uint32_t billed_cpu_time_us , uint32_t max_cpu_usage_ms, uint32_t max_block_cpu_ms,
   ...
   auto fut = transaction_metadata::start_recover_keys( std::move( ptrx ), test.control->get_thread_pool(),
                                                        ...
                                                        trx_type );
   auto res = test.push_transaction( fut.get(), fc::time_point::now() + fc::milliseconds(max_block_cpu_ms),
...
BOOST_FIXTURE_TEST_CASE(transaction_tests, TESTER) { try {
   ...
   // TODO: if transaction struct changed, must modify this size value
   CALL_TEST_FUNCTION(*this, "test_transaction", "test_transaction_size", fc::raw::pack(63) ); // TODO: Need a better way to test this.
   ...
   // test read_transaction only returns packed transaction
   {
      ...
      auto r = push_transaction( fut.get(), fc::time_point::maximum(), fc::microseconds::maximum(), DEFAULT_BILLED_CPU_TIME_US, true, 0 );
...
BOOST_FIXTURE_TEST_CASE(deferred_transaction_tests, TESTER) { try {
   ...
   set_code( "testapi"_n, test_contracts::test_api_wasm() );
   set_code( "testapi2"_n, test_contracts::test_api_wasm() );
   produce_blocks(1);
   ...
   //schedule twice without replace_existing flag (second deferred transaction should replace first one)
   {
      ...
      for (const auto& trx: dtrxs) {
         push_scheduled_transaction(trx, fc::time_point::maximum(), fc::microseconds::maximum(), 0, false);
      }
   ...
   //schedule twice with replace_existing flag (second deferred transaction should replace first one)
   {
      ...
      BOOST_CHECK_EQUAL(dtrxs.size(), 1);
      for (const auto& trx: dtrxs) {
         push_scheduled_transaction(trx, fc::time_point::maximum(), fc::microseconds::maximum(), billed_cpu_time_us, true);
      }
      BOOST_CHECK_EQUAL(1, count);
      BOOST_CHECK(trace);
      BOOST_CHECK_EQUAL( 1, trace->action_traces.size() );
...
BOOST_FIXTURE_TEST_CASE(permission_tests, TESTER) { try {
   ...
   CALL_TEST_FUNCTION( *this, "test_permission", "check_authorization",
      fc::raw::pack( check_auth {
         ...
         .permission = "active"_n,
         .pubkeys    = {
            public_key_type(string("FO7GfRtyDWWgxV88a5TRaYY59XmHptyfjsFmHHfioGNJtPjpSmGX"))
         }
      })
   );
...
BOOST_FIXTURE_TEST_CASE(resource_limits_tests, TESTER) {
   ...
   push_action( "flon"_n, "setpriv"_n, "flon"_n, mutable_variant_object()("account", "rlimits"_n)("is_priv", 1));
   produce_block();
