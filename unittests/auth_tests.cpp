...
BOOST_AUTO_TEST_CASE(link_auths) { try {
   ...
   chain.link_authority(name("alice"), name("flon"), name("spending"), name("reqauth"));
   ...
   BOOST_CHECK_THROW( chain.link_authority(name("alice"), name("flon"), name("spending"), name("reqauth")), action_validate_exception);

   // Unlink alice with eosio reqauth
   chain.unlink_authority(name("alice"), name("flon"), name("reqauth"));
   ...
   chain.link_authority(name("alice"), name("flon"), name("scud"));
...
BOOST_AUTO_TEST_CASE(link_then_update_auth) { try {
   ...
   chain.link_authority(name("alice"), name("flon"), name("first"), name("reqauth"));
...
try {
   ...
   // Creating account with flon. prefix with privileged account
   chain.create_account(name("flon.test1"));

   // Creating account with eosio. prefix with non-privileged account, should fail
   BOOST_CHECK_EXCEPTION(chain.create_account(name("flon.test2"), name("joe")), action_validate_exception,
                         fc_exception_message_is("only privileged accounts can have names that start with 'flon.'"));
...
BOOST_AUTO_TEST_CASE( any_auth ) { try {
   ...
   chain.link_authority( name("alice"), name("flon"), name("flon.any"), name("reqauth") );
   chain.link_authority( name("bob"), name("flon"), name("flon.any"), name("reqauth") );
